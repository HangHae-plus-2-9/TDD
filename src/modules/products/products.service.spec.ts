...
describe('ProductsService', () => {
  ...
  it('should throw an error if product name is empty', async() => {
    ...
    await expect(service.add_product_validation(storedProductInfo)).rejects.toThrow('Validation Error: Product name is required.');
});
  ...
  it('should throw an error if product price is minus', async() => {
    ...
    await expect(service.add_product_validation(storedProductInfo)).rejects.toThrow('Validation Error:product price is minus');
  });
  ...
  it('should throw an error if product price is not a natural number', async() => {
    ...
    await expect(service.add_product_validation(storedProductInfo)).rejects.toThrow('Validation Error:product price is not natural number');
  });
  ...
  it('should throw an error if product description is too long', async() => {
    ...
    await expect(service.add_product_validation(storedProductInfo)).rejects.toThrow('Validation Error: Product description is too long.');
});
  // post 유효성검사: 카테고리가 실제로 데이터베이스 내에 존재하지 않을때 에러 발생
  it('should throw an error if product category does not exist', async() => {
    const productWithNonExistingCategory = { name: 'Sample Product', price: 100, category: 'UnknownCategory' };
    mockRepo.create.mockResolvedValue(productWithNonExistingCategory);

    await expect(service.add_product_validation(productWithNonExistingCategory)).rejects.toThrow('Validation Error: Provided category does not exist.');
});  
});
